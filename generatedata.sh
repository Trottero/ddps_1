...
do
    ...
    $HIVE_HOME/bin/hive -e "SET mapred.max.split.size=128000000; SET mapreduce.input.fileinputformat.split.maxsize=128000000; INSERT OVERWRITE TABLE grep_${i}_select SELECT * FROM grep_${i} WHERE field LIKE '%XYZ%';" &
    sleep 14
done
wait
cd ~